...
Files <- function(){
  ...
  f11 <- read.csv("~/Google Drive/SAFMOD/Data/Ops_Workablehours.csv")
  f12 <- read.csv("~/Google Drive/SAFMOD/Data/Lab_Workablehours.csv")
  f13 <- suppressWarnings(read.csv("~/Google Drive/SAFMOD/Data/OpCost_Obj_Fun.csv"))
  f14 <- read.csv("~/Google Drive/SAFMOD/Data/Relative_change.csv")
...
cropData <- function(soil){ 
  ...
  if(missing(soil)){
    ...
    oy <- outputYield(nfs) # Output and margin estimates
...
objOpsCost <- function(soil){
  ...
  fa <- farmSoil(soil)
  nfa <- fa$Value[[2]]
  op <- operationCost(nfa)
  yp <- as.matrix(yieldPenalty(nfa)[,c(-1,-2)])
  ct <- -1 
...
workersMatrix <- function(){ 
  ...
  #===Labour
  wt[seq(73,98),9] <- macn*opw$Wkhrs[1:26]*cv*1.30#02
